...
async def list_restaurants(db: Session = Depends(get_db)):
    ...
    rests = [
        {"id": rest.id, "name": rest.name, "address": rest.address} for rest in all_rest
    ]
...
async def upload_restaurant_image(restaurant_name: str, image: UploadFile = File(...)):
    try:
        image_path = Path(
            f"assets/{restaurant_name}.jpg"
        )  # Adjust the path and naming as needed
...
async def restaurant_menu(restaurant_id: int, db: Session = Depends(get_db)):
    ...
    items = (
        db.query(RestaurantItem)
        .filter(RestaurantItem.restaurant_id == restaurant_id)
        .all()
    )

    if not items:
        raise HTTPException(
            status_code=404, detail="No items found for this restaurant"
        )
...
async def add_menu_item(
    restaurant_id: int, item: RestaurantItemBase, db: Session = Depends(get_db)
):
...
async def update_menu_item(
    ...
    item = (
        db.query(RestaurantItem)
        .filter(RestaurantItem.id == item_id)
